...
public interface IndividualCustomerRepository extends JpaRepository<IndividualCustomer, UUID>, JpaSpecificationExecutor<IndividualCustomer> {
    ...
    default List<IndividualCustomer> searchIndividualCustomer(SearchIndCustomerRequest request) {
        return findAll((root, query, criteriaBuilder) -> {
            ...
            var contactMediumJoin = root.join("id");
            var billingAccountJoin = root.join("billingAccountList");
...
//                predicates.add(criteriaBuilder.equal(contactMediumJoin.get("mobilePhone"),request.getMobilePhone())); // Long karşılaştırma
//            }
//            predicates.add(criteriaBuilder.like(criteriaBuilder.lower(root.get("mobilePhone")), "%" + (request.getMobilePhone() != null ? request.getMobilePhone() : "") + "%"));
